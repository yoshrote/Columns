...
from lxml.html import clean, soupparser
import re, urlparse, cgi, urllib2
from columns.lib import json
__all__ = ['stripobjects','striphtml','get_metamedia_data','get_post_thumbnail','get_embed_data']
re_nl = re.compile(ur'\n|\t|\r|&nbsp;')
...
def stripobjects(content):
	if not isinstance(content, basestring):
		return u''
	parser = etree.XMLParser(recover=True,resolve_entities=True,dtd_validation=False,no_network=False)
