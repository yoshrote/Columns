...
from columns.lib.exc import NoParentException

class TestAtompubController(unittest.TestCase):
	def test_unimplemented(self):
		ctrl = AtompubController()
		self.assertRaises(NoParentException, ctrl._get_parent_attr)
		self.assertRaises(NotImplementedError, ctrl._index,{})
		self.assertRaises(NotImplementedError, ctrl._create,'html')
		self.assertRaises(NotImplementedError, ctrl._new)
		self.assertRaises(NotImplementedError, ctrl._update,None,'html')
		self.assertRaises(NotImplementedError, ctrl._delete,None,)
		self.assertRaises(NotImplementedError, ctrl._get_from_id,1)
...
->			raise InvalidForm(htmlfill.render(resp, defaults=params, errors=errors, **htmlfill_kwargs2), errors)
	...
	def _index(self, spec, limit=None, offset=None):
	...
	def index(self, parent_id=None, format='html'):
		...
		try:
			...
			page_num = 1
		limit = settings.get('maximum_items') or 20
		offset = (page_num - 1) * limit
		
		spec = {}
		if parent_id is not None:
			parent_attr = self._get_parent_attr()
			spec[parent_attr] = parent_id
		
		items = self._index(spec, limit=limit, offset=offset)
